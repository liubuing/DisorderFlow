...
OUT = ROOT / 'results/pae_public_pilot_v10'
...
SEEDS = [20260918, 20261918, 20262918, 20263918]
...
def selected_sequences(component, pool):
    return sorted(pool, key=lambda s:hashlib.sha256(f'20260918|{component}|{s}'.encode()).hexdigest())[:20]
...
def prepare():
    ...
    freeze(OUT / 'protocol.json', {
        ...
        'candidate_rule':'all128 draws percomponent; remove native and exact duplicates; SHA256(20260918|component|sequence) first20; require at least10 for EVERY component; otherwise stop before labels',
...
def select_and_predict():
    ...
    entities = build_entities({'selections':selections},hold,20260918)
