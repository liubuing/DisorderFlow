...
def build_abstract_pdf():
    ...
    c.setFont('MSYHBD', 11)
    c.setFillColor(DARK_TEXT)
    c.drawString(comp_x + 10, PAGE_H - 145, "T2（失败）vs T2.1 v2（成功）")
    ...
    c.setFont('MSYH', 10)
    c.setFillColor(GREEN_TEXT)
    c.drawString(70, PAGE_H - 298, "门控通过：96.2% 有效结构, 100% 阳性接触恢复, 均值恢复 0.581")

    c.draw_footnote(
        "注释：T2.1 v2 使用确定性扭转扰动替代 T2 的高温扰动，确保结果可精确复现。"
        "肽约束力从 25 提升至 100 kJ/mol/nm² 后有效结构比例从 43% 跃升至 96.2%。"
    )
    c.showPage()

    # ==================== 第7页：4HIX 设计 ====================
    c.draw_slide_bg()
    c.draw_section_title(6, "4HIX IDP 抗体设计验证")
    c.draw_footer(7)
    ...
    evidence = [
        ...
        ("3. T2.1 v2 恢复", "n=31 结构, 96.2% 有效", "物理接触恢复（0.581）"),
    ...
    bugs = [
        ...
        "3. T2.1 扰动：肽约束力 25 → 100 kJ/mol/nm² 使有效比例从 43% 提升至 96.2%。",
    ]
    by = bug_y + 62
    for b in bugs:
        if b.startswith("   "):
    ...
    conclusions = [
        ...
        "T2.1 v2：96.2% 有效结构, 100% 阳性接触恢复（0.581）",
