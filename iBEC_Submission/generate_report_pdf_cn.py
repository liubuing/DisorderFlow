...
def build_report():
    ...
    elements.append(Paragraph(
        "主要结果包括：（1）<strong>表位条件似然位移（Epitope-Conditioned Likelihood Shift, ECLS）</strong>"
        "指标在 46 个抗原簇上展示了阳性原生序列 vs. 打乱序列判别能力（均值优势 0.217，80.4% 阳性簇）；"
        "（2）T2.1 v2 总体有效比例为 80.6%，但随机约束恢复高于 supplied contacts；"
        "（3）4HIX 描述性案例中，最优 ProteinMPNN 候选 ipTM 为 0.462，天然为 0.449。"
        "这些结果不证明接触特异恢复或前瞻性设计成功。",
        styles['CNBody']
    ))
    add_footnote(
    ...
    elements.append(Spacer(1, 6))
    elements.append(Paragraph(
        "supplied-contact 臂恢复为正，但随机约束均值更高（0.619 对 0.581）。"
        "这些对照不支持接触特异的恢复机制。",
        styles['CNBody']
    ))
    add_footnote(
        "脚注⑦：总体有效比例为 25/31（80.6%）；25/26（96.2%）仅是命中扰动层级后的条件 QC 比例。"
    ...
    elements.append(Spacer(1, 6))
    elements.append(Paragraph(
        "前 3 个 ProteinMPNN 候选在该次 AF2 运行中描述性地高于天然序列；"
        "该筛选后差异不能证明界面质量或结合改善。",
        styles['CNBody']
    ))
    add_footnote(
    ...
    elements.append(Paragraph(
        ...
        "（3）T2.1 v2 扭转恢复（n=31 结构, 80.6% 总体有效）——不支持接触特异机制；"
        "（4）4HIX ProteinMPNN 案例（n=20 设计）——筛选后的 AF2 描述性差异；"
