...
def extract_seq_from_pdb(pdb_path, chain_id):
    ...
    try:
        ...
        return ''
    sorted_resseqs = sorted(residues.keys())
    return ''.join(residues[r] for r in sorted_resseqs)


def build_multimer_fasta(ab_seq, ag_seq, ab_name='antibody', ag_name='antigen'):
    ab_seq = ab_seq.strip().upper()
    ag_seq = ag_seq.strip().upper()
    return f'>complex_{ab_name}_{ag_name}\n{ab_seq}:{ag_seq}\n'


def compute_interface_pae(pae_matrix, chain_a_len, chain_b_len=None):
    if chain_b_len is None:
        chain_b_len = pae_matrix.shape[0] - chain_a_len
    total = chain_a_len + chain_b_len
    if total > pae_matrix.shape[0]:
        total = pae_matrix.shape[0]
        chain_b_len = total - chain_a_len
    interface_pae = pae_matrix[:chain_a_len, chain_a_len:total]
    mean_pae = float(np.mean(interface_pae))
    return mean_pae, interface_pae


def score_binding_quality(contacts_summary, interface_metrics, af2_iptm=None,
                          interface_pae=None, complex_plddt=None):
    score = 0.0
    breakdown = {}
    if af2_iptm is not None:
        iptm_score = min(af2_iptm, 1.0)
        score += 0.35 * iptm_score
        breakdown['iptm_score'] = iptm_score
    else:
        breakdown['iptm_score'] = 0.0
    if interface_pae is not None:
        pae_clipped = min(interface_pae, 30.0)
        pae_score = max(0.0, 1.0 - pae_clipped / 30.0)
        score += 0.25 * pae_score
        breakdown['interface_pae_score'] = pae_score
    else:
        breakdown['interface_pae_score'] = 0.0
    n_contacts = contacts_summary.get('contact_count', 0)
    contact_score = min(n_contacts / 50.0, 1.0)
    score += 0.15 * contact_score
    breakdown['contact_score'] = contact_score
    charge_comp = interface_metrics.get('charge_complementarity', 0.0)
    charge_score = (charge_comp + 1.0) / 2.0 if charge_comp else 0.5
    score += 0.15 * charge_score
    breakdown['charge_score'] = charge_score
    hydrophobic_score = interface_metrics.get('hydrophobic_matching', 0.0)
    h_score = min(abs(hydrophobic_score), 1.0)
    score += 0.10 * h_score
    breakdown['hydrophobic_score'] = h_score
    breakdown['total'] = score
    return max(0.0, min(score, 1.0)), breakdown


def format_binding_quality_report(binding_score, breakdown, contacts_summary,
                                   interface_metrics, af2_iptm, interface_pae,
                                   complex_plddt):
    lines = []
    lines.append('=' * 60)
    lines.append('  Interface Binding Quality Assessment')
    lines.append('=' * 60)
    if binding_score >= 0.7:
        ...
        label = 'POOR'
    lines.append(f'  Overall Score: {binding_score:.3f} [{label}]')
    lines.append('')
    lines.append('  -- AF2 Confidence --')
    if af2_iptm is not None:
        lines.append(f'  ipTM: {af2_iptm:.3f}')
    if interface_pae is not None:
        lines.append(f'  Interface PAE: {interface_pae:.2f} A')
    if complex_plddt is not None:
        lines.append(f'  Complex pLDDT: {complex_plddt:.1f}')
    lines.append('')
    lines.append('  -- Interface Chemistry --')
    lines.append(f'  Contact residues: {contacts_summary.get("contact_count", 0)}')
    lines.append(f'  Interface size (A): {contacts_summary.get("interface_size_a", 0)}')
    lines.append(f'  Interface size (B): {contacts_summary.get("interface_size_b", 0)}')
    if interface_metrics:
        if 'hydrophobicity_a' in interface_metrics:
            lines.append(f'  Hydrophobicity A: {interface_metrics["hydrophobicity_a"]:+.3f}')
        if 'hydrophobicity_b' in interface_metrics:
            lines.append(f'  Hydrophobicity B: {interface_metrics["hydrophobicity_b"]:+.3f}')
        if 'charge_complementarity' in interface_metrics:
            lines.append(f'  Charge complementarity: {interface_metrics["charge_complementarity"]:.3f}')
        if 'hbond_potential' in interface_metrics:
            lines.append(f'  H-bond potential: {interface_metrics["hbond_potential"]}')
    lines.append('')
...
def format_epitope_summary_for_ui(epitope_data, top_n=10):
    if not epitope_data:
        return '*No epitope data available.*'
    lines = [
        '| Rank | Residue | ResSeq | SASA | relSASA | Hydro | Protr | Combined |',
        '|------|---------|--------|------|---------|-------|-------|----------|',
    ]
    for item in epitope_data[:top_n]:
        rank = item.get('rank', '?')
        resname = item.get('resname', '?')
        resseq = item.get('resseq', '?')
        sasa = item.get('sasa', 0)
        rel_sasa = item.get('rel_sasa', 0)
        hydro = item.get('hydrophilicity', 0)
        protr = item.get('protrusion_index', 0)
        combined = item.get('combined_score', 0)
        lines.append(
            f'| {rank} | {resname} | {resseq} | '
            f'{sasa:.1f} | {rel_sasa:.2f} | {hydro:+.2f} | {protr:.2f} | {combined:.3f} |'
        )
    return '\n'.join(lines)
