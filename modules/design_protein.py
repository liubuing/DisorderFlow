...
Usage Examples:
  # Design specific region on chain A (from existing PDB)
  python design_protein.py input.pdb --design "A:10-25" --config config.yml --device cpu

  # Design from FASTA (auto-generates PDB template)
  python design_protein.py --fasta seqs.fasta --design "A:10-25" --config config.yml

  # Design multiple regions across chains
  python design_protein.py input.pdb --design "A:10-25 B:5-15" --config config.yml
...
# Import PDB generation from app (shared Gradio app module)
...
def parse_design_regions(design_str):
    ...
    Format: "CHAIN:START-END CHAIN:R1,R2,R3" or "CHAIN:START-END"
...
def load_fasta(fasta_path):
    """Load sequences from a FASTA file. Returns list of (header, seq)."""
    seqs = []
        ...
        for line in f:
            ...
            if line.startswith('>'):
                if header and current_seq:
                    seqs.append((header, current_seq))
                header = line[1:]
                current_seq = ''
            elif line:
                current_seq += line
...
def fasta_to_pdb(fasta_path, out_dir=None):
    """Convert first FASTA entry to a PDB template file. Returns pdb_path."""
    ...
    if pdb_content is None:
        print('[ERROR] No valid amino acids in sequence')
        return None
    ...
    print(f'[INFO] Generated PDB template: {pdb_path} ({len(seq)} residues, alpha-helix backbone)')
    return pdb_path
...
def design_protein(args):
    ...
    if pdb_path is None and args.fasta:
        pdb_path = fasta_to_pdb(args.fasta)
    ...
    if args.output:
        output_data = {
            'pdb': pdb_id,
            'design_regions': {k: v for k, v in design_regions.items()},
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Design from existing PDB
  python design_protein.py input.pdb --design "A:10-25" --config config.yml

  # Design from FASTA (auto-generates PDB template with alpha-helix backbone)
  python design_protein.py --fasta sequence.fasta --design "A:10-25" --config config.yml
  ...
  python design_protein.py input.pdb --design "A:10-30" --config config.yml --eval --output results.json
    ...
    parser.add_argument('pdb_path', type=str, nargs='?', default=None,
                        help='Input PDB file (omit if using --fasta)')
    parser.add_argument('--fasta', '-f', type=str, default=None,
                        help='FASTA file with target sequence (generates PDB template)')
    ...
    if args.pdb_path is None and args.fasta is None:
        parser.error('Either pdb_path or --fasta must be provided')
    design_protein(args)
