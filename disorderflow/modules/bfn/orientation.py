...
from disorderflow.modules.common.geometry import normalize_vector
...
def svd_project_so3(F):
    ...
    # MPS/XPU/CUDA: torch.linalg.svd and det don't support float16.
    # Always force float32 and disable autocast for this region.
    if device.type in ('mps', 'xpu', 'cuda'):
        with torch.autocast(device_type=device.type, enabled=False):
            F32 = F.float()
            U, S, Vh = torch.linalg.svd(F32)
        ...
        F32 = F.float() if F.dtype != torch.float32 else F
        U, S, Vh = torch.linalg.svd(F32)
        d = torch.linalg.det(U @ Vh)
        mid = torch.ones_like(S)
        mid[..., -1] = d
