...
from disorderflow.modules.common.geometry import angstrom_to_nm, pairwise_dihedrals
from disorderflow.modules.common.layers import AngularEncoding
from disorderflow.utils.protein.constants import BBHeavyAtom, AA
...
class PairEmbedding(nn.Module):
    ...
    def forward(self, aa, res_nb, chain_nb, pos_atoms, mask_atoms, structure_mask=None, sequence_mask=None, generate_flag=None):
        """
        Args:
            aa: (N, L).
            res_nb: (N, L).
            ...
            pos_atoms:  (N, L, A, 3)
            mask_atoms: (N, L, A)
            structure_mask: (N, L), 1D mask for structure leakage
            sequence_mask:  (N, L), mask out unknown amino acids to generate.
            generate_flag: (N, L) bool, if provided, allows cross-chain pairs
                           involving generated residues (for epitope conditioning).
        ...
        # Allow cross-chain pairs for epitope conditioning: generated residues
        # on one chain can see context residues on another chain.
        if pair_structure_mask is not None and generate_flag is not None:
            cross_chain = (chain_nb[:, :, None] != chain_nb[:, None, :])
            pair_structure_mask = pair_structure_mask | cross_chain
