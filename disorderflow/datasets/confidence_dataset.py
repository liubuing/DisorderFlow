...
class ConfidenceRegressionDataset(Dataset):
    ...
    def __init__(self, cfg, transform=None, **kwargs):
        ...
        if self.max_residues > 0 and self._use_lmdb:
            ...
            if n_filtered > 0:
                print(f'[ConfidenceRegression] Filtered {n_filtered} entries > {self.max_residues} residues '
        ...
        if self.min_plddt > 0.0:
            ...
            if n_removed > 0:
                print(f'[ConfidenceRegression] Quality filter (>={self.min_plddt} pLDDT): '
        ...
        if self.idp_sample_weight > 1.0:
            ...
            for idx, real_idx in enumerate(self._valid_indices):
                ...
                if entry.get('is_idp', False):
                    ...
                    n_idp += 1
            if n_idp > 0:
                print(f'[ConfidenceRegression] IDP-weighted sampling: {n_idp} IDP entries '
    ...
    def __getitem__(self, index):
        ...
        batch['scaffold_id'] = torch.tensor(int(entry.get('scaffold_id', real_idx)),
                                            dtype=torch.long)
        ...
        if disorder_mask is not None:
            ...
            batch['disorder_label'] = torch.ones(seq_len, dtype=torch.float32) if is_idp else torch.zeros(seq_len, dtype=torch.float32)
